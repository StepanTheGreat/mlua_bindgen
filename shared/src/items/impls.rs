...
pub fn parse_field(input: ImplItemFn, kind: FieldKind) -> syn::Result<ParsedField> {
    ...
    if user_arg_count != kind.user_args() {
        let msg = match kind {
            FieldKind::Getter => format!("Getters can't contain more than 2 default arguments (&Lua and &Self)"),
            FieldKind::Setter => format!("Setter have to contain exactly 3 arguments (&Lua, &mut Self and 1 user argument)")
        };
        return Err(syn_error(
            func.name, 
            msg
